...
idx = nydmvVinDecodeDf[( nydmvVinDecodeDf["ABS"].isin([
    ...
    nydmvVinDecodeDf["RearCrossTrafficAlert"].isin(['UNITED STATES (USA)']) | nydmvVinDecodeDf["VIN"].isna() ) & ~nydmvVinDecodeDf["ORIG_VIN"].isna()].index
if len(idx) > 0:
...
print("Write new corrected csv with desired columns")
nydmvVinDecodeDf.to_csv("tmp/nydmvvin-goodhdr/NYDMV-VIN-OUTPUT-merged-fixedquotes-cols.csv",index=False)
